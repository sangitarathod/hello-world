...
add_filter( 'the_content', 'layout_conetnt_function' );
 
function layout_conetnt_function( $content ) {
 
    if(isset($_GET['ly'])){
		$ly=$_GET['ly'];
		
		$content=get_string_between($content, "<layout $ly>", "</layout>");
...
//////////
add_filter( 'woocommerce_order_formatted_line_subtotal', 'filter_function_name_9032', 10, 3 );
function filter_function_name_9032( $subtotal, $item, $this ){
	// filter...

	return $subtotal;
}

add_filter( 'woocommerce_order_item_name', 'filter_function_name_9413', 10, 3 );
function filter_function_name_9413( $item_name, $item, $false ){
...
/////
add_action('woocommerce_before_order_table', 'the_email_id_as_a_global', 1, 4);
