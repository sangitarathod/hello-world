...
function barcode( $filepath="", $text="0", $size="20", $orientation="horizontal", $code_type="code128", $print=false ) {
	...
	$image = imagecreate($img_width * 4, $img_height + $text_height);
	...
	$location = 10;
	for ( $position = 1 ; $position <= strlen($code_string) * 4; $position++ ) {
		$cur_size = $location + ( substr($code_string, ($position-1), 1) * 4 );
		if ( strtolower($orientation) == "horizontal" )
