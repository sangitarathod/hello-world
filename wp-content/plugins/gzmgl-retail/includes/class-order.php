...
class Gzmgl_Retail_Order {
	...
	static function init() {
		...
		add_shortcode('order-analysis',__CLASS__. '::order_analysis' );
	...
	static function order_analysis(){
		$html="<div class='analysis'>
		<h3>Trade-in weekly/monthly</h3></div>";
		return $html;
	}
	...
	static function order_list() {
		...
		if ( ! empty( $form['fields'] ) ) {
			...
			<table class="order-list">
				<thead>
					<tr>
						<th><?php _e( 'Dates', 'gzmgl_retail' ); ?></th>
				...
				<tbody>
					<?php foreach ( $entries as $entry_id ) :
						...
						///////////////////////////////////////////////////
						$dev_list = '';
						foreach( explode( '~', $values['device']['value'] ) as $dev ){
							foreach( explode( '^', $dev ) as $attr ){
								if( preg_match( '/\d,\d/', $attr ) ){ $p = explode( ',', $attr ); $attr = '$'.$p[0]; }
								$dev_list .= $attr."<br/>";
							}
							$dev_list .= "<br/>";
						}
					?>
						<tr>
							...
							<td style='align:left;'><?php echo $dev_list?><?php //echo $values['device']['value']; ?></td>
			...
			<div class="button_row">
				...
				<a class="button hide_from_print" href="http://labels.gizmogul.com/print_retail.php?id=<?php echo get_current_user_id(); ?>" target="_blank"><?php _e( 'Download FEDEX Shipping Label', 'gzmgl_retail' ); ?></a>
				<a class="button hide_from_print" href="http://labels.gizmogul.com/print_retail_usps.php?id=<?php echo get_current_user_id(); ?>" target="_blank"><?php _e( 'Download USPS Shipping Label', 'gzmgl_retail' ); ?></a>
		...
		<?php
			...
			echo $result;?>
			<h3>Total devices traded</h3>
			<?php 
			$search_criteria2 = array(
		    'status'        => 'trash',
		    
			);
			

			$search_criteria2['field_filters'][] = array( 'key' => 'created_by', 'value' => get_current_user_id() );
		
			$result2 =GFAPI::count_entries( $form_id, $search_criteria2 );
	...
	static function overview_html( $entry, $form, $only_labels = false, $pdf = false ) {
		...
		$address .= $values['document-type']['value'] . '<br/><br/>';
		
		//$img = self::generate_barcode_image( '7777777' , '7777777'.$_GET['c'] );
		$bar = $_GET['c'];
		if( $_GET['entry_id'] > 0 ){ $bar = $_GET['entry_id']; }
		//else{ $sz = 'width=110 height=100'; }
		$img = 'http://labels.gizmogul.com/barcode.php?codetype=Code128&size=100&text=X'.$bar;
		$address .= "<div class=\"sticker-label\"><img src=$img $sz></div><br/>";
		//$address .= "<div class=\"sticker-label\"><img src=$img width=110 height=100></div><br/>";
		...
		<div class="order-overview">
			...
			<table class="order-details">
				<thead>
					<tr>
						<th><?php _e( 'Device(s)', 'gzmgl_retail' ); ?></th>
					...
					<tr>
						<td><?php
							...
							if ( ! empty( $values['serial']['value'] ) && 'false' != $values['serial']['value'] ) {
								echo '<p>IMEI #: ' . $values['serial']['value'] . '</p>';
								echo '<p>Suggested Model:'.WC()->session->get( 'imei-'.$values['serial']['value']).'</p>';
							}
						?></td>
			...
			<br/>
			<h2><img class="image" src="https://retail.gizmogul.com/wp-content/themes/gizmogul-retail/images/long-arrow-pointing-to-the-right.png">&nbsp;&nbsp;&nbsp;Please Print and Sign</h2>
			<p>In order to complete a Product "trade-through" via Gizmogul Recycling Service, Customer must accept the following Terms and Conditions as stated herein.</p>
			<p>Customer represents and warrants as follows:</p>
			<p>1.  Customer is the sole owner of the Product or has been authorized by the owner of the Product to make all decisions with regard to the disposition of the Product, and there are no liens, or security interests in or attached to the Product and that no other party has a legal interest in it. Buyer may take reasonable actions, including utilizing a national lost or stolen goods database, to determine if submitted items were ever reported lost or stolen. Products provided and listed or reported as stolen may be reported to applicable law enforcement agencies, and Customer acknowledges and grants the full right and authority to release any Customer contact information provided through the Recycling Services to applicable law enforcement agencies for further investigation.</p>
			<p>2. The Product is not counterfeit, lost, stolen or fraudulent.</p>
			<p>3.  Any item Customer seeks to recycle has not been modified by Customer and shall not infringe on any third-party intellectual property right (including copyright, trademarks, patent, trade secrets or other proprietary right).
			</p>
			<p>4. Upon physical receipt of the Product and issuance of the Visa Prepaid Gift Card, title and ownership in such Product transfers to Buyer and Customer disclaims any further right, title or interest in and to the Product or any items or data contained therein.
</p>
			<p>5.  Customer acknowledges that it is his or her responsibility to remove all personal data from his/her Product, before selling, including SIM card, removable memory cards, and must data clear the Product, to include a factory reset. Buyer is not responsible or liable for the removal of Customer’s personal data.</p>
			<p>6.  Customer agrees that he/she has the sole responsibility, if so desired, to keep a separate back up copy of any files or data before selling the Product; and that Customer has taken reasonable steps to eliminate and delete files and data that are deemed personal or confidential. Data recovery is not a part of the Recycling Services and Buyer accept no responsibility or liability for any lost files or data leakage.
			</p>
			<p>7. This Recycling Service is provided for lawful purposes only, to the extent permitted by law, Customer agrees to indemnify Buyer and any of its directors, officers, employees, affiliates, subsidiaries or agents from and against any claims brought against any of them arising from Customer’s breach of the terms and conditions of the Recycling Services.
</p>
			<p>8.  This Recycling Service is void where prohibited or restricted by law.</p>
			<p>9.  The Product quote provided is based on the Customer’s full, truthful and accurate responses to the qualification questions as to Product type, specifications and conditions.
</p>
			<p>10. If an online trade initiates in store the customer will receive payment in the form of a Visa Gift Card. The Visa Gift Card will expire after two years and unused funds will be forfeited.  </p>
			<p>The Recycling Service is provided to Retail Customers by Gizmogul, Inc. as an independent third party company. Retailer is not a party in the transaction between Gizmogul, Inc. and the Recycling Service Customers. Any and all transactions are solely the responsibility of Gizmogul, Inc. and Customer. YOU ACKNOWLEDGE AND AGREE THAT THE RECYCLING SERVICE IS BETWEEN YOU AND GIZMOGUL, INC. ONLY, AND RETAILER IS NOT LIABLE FOR AND DOES NOT WARRANT SUCH SERVICE. By using the Recycling Service, you hereby release Retailer and its respective directors, employees and agents from any disputes, claims, demands, and/or damages (actual or consequential) of every kind, whether known or unknown, arising out of, or relating to, your use of Gizmogul, Inc. Recycling Service, including, without limitation, incomplete or completed transactions and any claims or disputes between you and Gizmogul, Inc.</p>
			<?php endif; ?>
	...
	public static function generate_overview_pdf_ajax() {
		...
		if ( ! $only_labels ) {
			...
			echo '<p>Signature: ____________________ &nbsp; &nbsp; Date: ____________________</p></div>';
		}
		$data = ob_get_clean();
