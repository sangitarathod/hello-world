...
class WCFMmp_Refund {
    ...
    add_action('wp_ajax_wcfmmp_refund_requests_form_html', array( &$this, 'wcfmmp_refund_requests_form_html' ) );
		...
		add_filter( 'wcfmmarketplace_orders_actions', array( &$this, 'wcfmmp_refund_orders_actions' ), 100, 3 );
	}
  ...
  function wcfm_refund_endpoint_title( $title, $endpoint ) {
  	...
  	switch ( $endpoint ) {
  		case 'wcfm-refund-requests' :
				$title = __( 'Refund Requests', 'wc-frontend-manager-ultimate' );
			break;
  	}
  ...
  function wcfmmp_refund_requests_form_html() {
	...
	public function wcfmmp_refund_orders_actions( $actions, $vendor_id, $order ) {
  	global $WCFM, $WCFMmp;
		...
		if( $status == 'completed' ) {
			...
			if( !empty( $refund_infos ) ) {
				foreach( $refund_infos as $refund_info ) {
					...
					if( $wc_refund_processed && $commission_id ) {
						$commission_amount = 0;
						$total_commission = 0;
						
						// Fetch Commission details & recalculate commission
						$sql = 'SELECT product_id, variation_id, item_sub_total, item_total, quantity, commission_amount, total_commission, refunded_amount, tax, shipping, shipping_tax_amount, commission_status FROM ' . $wpdb->prefix . 'wcfm_marketplace_orders';
						$sql .= ' WHERE 1=1';
						...
						if( !empty( $commission_infos ) ) {
							foreach( $commission_infos as $commission_info ) {
								...
								if( $is_partially_refunded ) {
									...
									$commission_amount = $WCFMmp->wcfmmp_commission->wcfmmp_get_order_item_commission( $vendor_id, $commission_info->product_id, $commission_info->variation_id, $item_total, $commission_info->quantity );
			...
			if( !empty( $refund_infos ) ) {
				foreach( $refund_infos as $refund_info ) {
					...
					do_action( 'wcfmmp_commission_refund_status_'.$status, $refund_id, $commission_id );
					...
					do_action( 'wcfmmp_refund_status_'.$status, $refund_id );
